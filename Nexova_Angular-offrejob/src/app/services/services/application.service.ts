...
export class ApplicationService {
  ...
  createApplicationWithFiles(formData: FormData, jobOfferId: number): Observable<Application> {
    return this.http.post<Application>(`${this.baseUrl}/joboffer/${jobOfferId}`, formData);
  }
...
downloadCoverLetter(fileName: string): Observable<Blob> {
  return this.http.get(`${this.baseUrl}/cover-letter/${fileName}`, { responseType: 'blob' });
}

// Download resume
downloadResume(fileName: string): Observable<Blob> {
  return this.http.get(`${this.baseUrl}/resume/${fileName}`, { responseType: 'blob' });
}

updateInterviewSchedule(id: number, interviewSchedule: string): Observable<Application> {
  return this.http.put<Application>(`${this.baseUrl}/${id}/interview-schedule`, interviewSchedule);
}
// Download resume
